...
router.post('/packaging-detail', (req, res) => {
  ...
  db.all(sql, packagingIds, (err, items) => {
    ...
    const reportRows = items.map(item => {
      ...
      return {
        id: item.id, product_name: item.product_name, manufacturer: item.manufacturer,
        material: item.material_name, category: item.category,
        recyclable: !!item.recyclable, biodegradable: !!item.biodegradable,
        weight_kg: weight, cost_gbp: cost,
        epr_cost_gbp: weight * EPR_RATE, ppt_cost_gbp: weight * PPT_RATE,
        carbon_emissions_kg: weight * CO2_PER_KG,
    ...
    if (format === 'csv') {
      const csv = [
        ...
        ...reportRows.map(r => [
          r.id, r.product_name, r.material, r.category, r.weight_kg, r.cost_gbp,
          r.epr_cost_gbp, r.ppt_cost_gbp, r.carbon_emissions_kg,
